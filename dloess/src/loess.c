...
void
	loess_setup(x, y, n, p, lo) double *x,
	*y;
int32_t n, p;
...
void
	loess(lo) struct loess_struct *lo;
{
	...
	iterations = (!strcmp(lo->model.family, "gaussian")) ? 0 : lo->control.iterations;
...
void
	loess_(y, x_, size_info, weights, span, degree, parametric, drop_square,
		   ...
		   parameter, a, xi, vert, vval) double *y,
	*x_, *weights, *span, *cell, *pseudovalues,
	*fitted_values, *fitted_residuals, *enp, *s, *one_delta, *two_delta,
...
char **statistics, **surface, **trace_hat_in;
{
	...
	*one_delta = *two_delta = *trace_hat_out = 0;
	...
	for (i = 0; i < D; i++)
	{
		fprintf(stderr, "loess.c i=%d sum_drop_sqr=%d drop_square[i]=%d sum_parametric=%d\n", i, sum_drop_sqr, drop_square[i], sum_parametric);
	...
	for (i = 0; i < D; i++)
	{
		...
		{
			x[k + j] = x_tmp[p + j];
			fprintf(stderr, "k=%d, j=%d, x[k + j]=%f\n", k, j, x[k + j]);
		}
	}
	...
	for (j = 0; j <= (*iterations); j++)
	{
		...
		if (j < (*iterations))
			F77_SUB(lowesw)
			(fitted_residuals, &N, robust, temp);
	}
	if ((*iterations) > 0)
	{
		F77_SUB(lowesp)
		(&N, y, fitted_values, weights, robust, temp, pseudovalues);
...
void
	loess_free_mem(lo) struct loess_struct *lo;
{
	free(lo->in.x);
	free(lo->in.y);
	free(lo->in.weights);
...
void
	loess_summary(lo) struct loess_struct *lo;
{
...
void
	condition(surface, new_stat, trace_hat_in) char **surface,
	*new_stat, **trace_hat_in;
{
